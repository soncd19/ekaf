               ...
               true ->
                   case ekaf_server_lib:handle_pick({pick, Topic}, self(), State) of
                       ...
                       {NextWorker, NextState} when Strategy =:= strict_round_robin->
                           Members = pg:get_local_members(?PREFIX_EKAF(Topic)),
...
handle_info({worker, up, WorkerUp, WorkerUpStateName, WorkerUpState, _}, StateName, #ekaf_server { topic = Topic, messages = OfflineMessages } = State) ->
    pg:leave(?PREFIX_EKAF(Topic), self()),
    case StateName of
        ready ->
    ...
    fsm_next_state(StateName, Next#ekaf_server{ worker = WorkerUp, messages = [], workers = pg:get_local_members(?PREFIX_EKAF(Topic))});
